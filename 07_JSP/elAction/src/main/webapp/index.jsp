...
<body>
    ...
    <%--
    	*JSP를 이루는 구성
    	1. 스크립팅 원소 : jsp페이지 내에서 자바 코드를 사용할 수 있는 기술
    	ex) 스크립틀릿, 표현식...
    	
    	2. 지시어 : jsp페이지 정보에 대한 내용을 표현한다거나 또 다른 페이지를 포함할 때 사용
    			  jsp기능을 확장 시키는 라이브러리르 등록할 때 사용
    			  
    	3. 액션태그 : jsp페이지에서 어떤 동작을 하도록 지시하는 태그
    				xml기술을 이용하여 기존의 jsp문법을 확장하는 기술을 제공하는 태그
    	   >> 표준액션태그 : jsp페이지에서 바로 사용이 가능(별도의 연동 필요 x)
    	   			-> 모든 태그명 앞에 jsp: 접두어를 붙여서 사용 <jsp:태그명>
    	   >> 커스텀액션태그 : jsp페이지에 바로 사용 불가능(별도의 라이브러리 연동이 필요)
    	   			-> 접두어에 jsp: 이 아닌 모든 액션태그(가장 많이 사용하는 라이브러리 jstl)
     --%>
     ...
     <h1>JSTL이란?</h1>
     <p>
     	jsp에서 자주 사용하는 기능들을 태그형태로 제공하는 표준 라이브러리(커스텀액션태그) <br>
     	자바코드를 직접 사용하지 않고도 반복문, 조건문, 포멧처리... 쉽게 수행할 수 있음 <br>
     	태그로 작성이 되어있기 때문에 코드 가독성이 높아짐.
     </p>
