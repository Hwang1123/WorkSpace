/*
    <INDEX>
    * 인덱스 : 데이터를 빠르게 검색하기 위한 데이터의 정렬과, 탐색같은 DBMS의 성능향상을  
              목적으로 생성하는 객체
    - 검색속도가 향상됩니다. (SELECT문에서 WHERE조건으로 데이터를 빠르게 검색)
        단, 모든 경우에 향상되는 것은 아니다.(INSERT, UPDATE, DELETE 작업시,
                                            인덱스도 함께 업데이트 되어 성능저하될 수 있음)
*/
...
COST : 실행예상 비용(자원을 얼마나 소비하는가) => 값이 낮을 수록 적은 비용으로 검색을 수행할 수 있음.
ROWS : 실행계획에서 ACCESS된 ROW수
BYTE : 실행계획에서 ACCESS된 BYTE수

TABLE ACCESS FULL(FULL스캔) : 전체 테이블을 탐색해서 결과를 도출하게 될 것이다.
*/
...
-- 두 가지 이상으로 조건조회를 자주할 때
-- 결합인덱스 생성
CREATE INDEX USER_INDEX_DATA_FIRSTNAME_ID ON USER_INDEX_DATA(ID, FIRST_NAME);
