...
/*  
    ACID속성 : 트랜잭션의 일관성과 신뢰성을 보장하기 위해 지켜야하는 네가지 핵심 송성
    원자성 : 트랜잭션 내의 작업이 모두 완료(COMMIT)되거나, 모두 취소(ROLLBACK)되어야 함.
    일관성 : 트랜잭션이 실행되지 전과 후에 데이터베이스의 상태가 항상 일관성을 유지해야 함.
    독립성 : 동시에 실행되는 트랜잭션이 서로에게 영향을 미치지 않도록 독립적으로 처리되어야 한다.
    지속성 : 트랜잭션이 성공적으로 완료된 후에는 시스템 장애가 발생하더라도 영구적으로 저장되어 있어야 함.
*/
