/*
    ...
    자동으로 매번 실행한 내용을 미리 정의해둘 수 있다.
    
    EX) 
    회원탈퇴 시 기존의 회원테이블 데이터를 DELETE한 후  곧바로 탈퇴한 회원들만 따로 보관하는 테이블에 
    자동으로 INSERT시켜야 한다.
    신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리하게끔.
    입출고에 대한 데이터 기록(INSERT)될 때마다 해당 상품에 대한 재고 수향을 매번 수정(UPDATE)해야한다.
    ...
    * 트리거의 종류
    ...
    - 문장트리거 : 이벤트가 발생한 SQL문에 대해 딱 한 번만 트리거 실행
...
-- EMPLOYEE테이블에 새로운 행이 INSERT 될 때마다 자동으로 출력되는 트리거 정의
...
-- TB_PRODETAIL테이블에 INSERT이벤트 발생 시
-- TB_PRODUCT테이블에 매번 자동으로 재고 수량을 UPDATE하게 트리거 생성

/*
    -- 상품이 입고 => 해당 상품 찾아서 재고수량 증가 UPDATE
        UPDATE TB_PRODUCT
        SET STOCK = STOCK + 입고된 수량(INSERT된 자료의 AMOUNT)
        WHERE PCODE = 입고된상품 번호(INSERT된 자료의 PCODE);
    
    
    -- 상품이 출고 => 해당 상품 찾아서 재고수량 감소 UPDATE
         UPDATE TB_PRODUCT
         SET STOCK = STOCK - 출고된 수량(INSERT된 자료의 AMOUNT)
         WHERE PCODE = 출고된상품 번호(INSERT된 자료의 PCODE);
