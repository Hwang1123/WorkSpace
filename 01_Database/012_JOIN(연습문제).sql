...
--8. 같은 부서에서 일하는 다른 사원의 이름을 조회하시오
--  단, 모든 사원이 자신의 부서에서 함께 일하는 다른 사원의 이름을 확인할 수 있어야 한다 
--  (EX : 사원 A가 속한 부서에서 다른 사원 B의 이름을 조회)
SELECT E.EMP_NAME, D.DEPT_CODE, D.EMP_NAME
FROM EMPLOYEE E
JOIN EMPLOYEE D ON(E.DEPT_CODE = D.DEPT_CODE)
WHERE E.EMP_ID != D.EMP_ID
ORDER BY E.EMP_ID; -- * 복습 *

--9. 보너스가 없고 직급 코드가 J4이거나 J7인 사원의 이름, 직급 명, 급여 조회(NVL 이용)
SELECT EMP_NAME, JOB_NAME, NVL(BONUS, SALARY) AS "급여"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) >= (SUM(SALARY) * 0.2);
...
FROM EMPLOYEE
JOIN DEAPRTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1, 6),'RR/MM/DD')) / 365) = 
    (MIN(FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1, 6),'RR/MM/DD')) / 365)) AS "나이"
    FROM EMPLOYEE);
