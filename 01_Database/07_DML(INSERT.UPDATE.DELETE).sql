...
/*
    ...
    부족하게 갑을 제시할 경우, 값을 더 많이 제시한 경우 => 에러발생
*/
SELECT * FROM EMPLOYEE;

INSERT INTO EMPLOYEE
VALUES(900, '이소근',  '880914-1456789',
        'SG8809NAVER.COM', '01078945656', 'D7', 'J5', 4000000, 0.2, 200, SYSDATE, NULL, 'N');
        
/*
    2)INSERT INTO 테이블명(컬럼, 컬럼, 컬럼...) VALUES(값, 값, 값, 값...)
    테이블에 내가 선택한 컬럼에 대한 값만 INSERT할 수 있다.
    그래도 한 생 단위로 추가 되기 때문에 선택 되지 않은 컬럼은 기본적으로 NULL이 들어감
    => NOT NULL 제약조건이 걸려있는 걸럼은 반드지 직접 값을 넣어줘야 한다.
    단, 기본값이 지정 되어 있으면 NULL이 아닌 기본값이 들어간다.
*/
...
/*
    ...
    VALUES에 직접 값을 명시하는 것 대신 서브쿼리로 조회 된 값을 통째로 INSERT가능
...
/*
    [표현식]
    INSSERT ALL
    INRO 테이블명1 VALUES(컬럼, 컬럼, 컬럼....)
    INRO 테이블명2 VALUES(컬럼, 컬럼....)
    서브쿼리;
*/
INSERT ALL
...
-- EMP_SALARY 테이블에서 노옹철 사원의 급여를 100만으로 변경
UPDATE EMP_SALARY
SET SALARY = 1000000
WHERE EMP_NAME = '노옹철';
...
WHERE EMP_NAME = '선동일';
-- EMP_SALARY 테이블에서 전체사원의 급여를 기본 급여 10% 인상된 금액으로 변경하라.
UPDATE EMP_SALARY
...
-- 방명수 사원의 급여와 보너스 값을 유재식 사원의 급여와 보너스 값으로 변경한다
UPDATE EMP_SALARY
...
-- ASIA 지역에서 근무하는 사원들의 보너스 값을 0.3을 연결
UPDATE EMP_SALARY
...
/*
    ...
    [표현식]
    DELETE FROM 테이블명
    [WHERE 조건] => WHERE절 제시 안하면 전헤행 다 삭제됨!
*/
