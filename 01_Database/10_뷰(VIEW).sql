/*
    ...
    (자주 사용하는 SELECT문을 저장해두면 긴 SELECT문을 매번 다시 기술 할 필요 없이 사용 할 수 있다.)
    가상 테이블 => 실제 데이터가 담겨 있는 것은 아니다(논리테이블)
*/

-- 한국에서 근무하는 사원들으 사번, 이름, 부서명, 급여, 근무국가명 조회
...
-- GRANT CREATE VIEW TO C##KH;  ==> 뷰 생성 권환
...
-- CREATE OR REPLACE
-- VIEW가 없을 때는 생성, 이미 존재한다면 수정 할 수 없다.
...
/*
    뷰 컬럼에 별칭 부여
    서브쿼리의 SELECT절에 함수식이나 산술연산식이 기술되어 있다면 반드시 별칭을 부여 해야한다.
*/
...
-- 부를 통해서 조작하게 되면 실제 데이터가 담겨있는 테이블에 반영된다.

CREATE OR REPLACE VIEW VM_JOB
AS (SELECT JOB_CODE, JOB_NAME
    FROM JOB);
    
SELECT *FROM VM_JOB; -- 논리테이블(실제 데이터가 담겨있지 않다.)
SELECT * FROM JOB;
...
SELECT * FROM VM_JOB;
SELECT * FROM JOB; -- 논리테이블(실제 데이터가 담겨있지 않다.)
----------------------------------------------------------------------------
/*
    * DML 명령어로 조작이 불가능한 경우
    1) 뷰에 정의되어 있지 않은 컬럼을 조작하려는 경우
    2) 부에 정의되어 있지 않은 컬럼 중에 데이터베이스 상에 NOT NULL제약조건이 지정되어 있는 경우
    3) 산술연산식이나 함수식을 사용한 경우
    4) 그룹함수나 GROUP BY절을 포함한 경우
    5) DISTINCT구문이 포함된 경우
    6) JOIN을 이용해서 여러 테이블을 연관 시켜놓은 경우 
...
/*
    ...
    FORCE | NOFORCE
        ...
        > NOFORCE : 서브쿼리에 기술된 테이블이 존재라는 테이블이여야만 한다.(기본값)
...
-- WITH READ ONRY : 뷰에 대해서 조회만 가능하도록 설정
CREATE OR REPLACE VIEW VM_EMP
