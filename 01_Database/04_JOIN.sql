/*
    ...
    관계형 데이터 베이스에서는 최소한의 데이터를 각각의 테이블에 담고 있음
    (무장정 다 조회해 오는 것이 아니라 각 테이블 간 연결고리(외래키)를 통해 데이터를 매칭시켜 조회해야 한다.)
...
*/
--> 오라클 전용구문
--  FROM절에 조회하고자 하는 테이블을 나영(,구분)
--  WHERE절에 매칭시킬 컬럼에 대한 조건을 제시

-- 1. 연결할 두 컬럼명이 다른 경우(EMPLOYEE : DPPT_CODE / DEPARTMENT : DEPT_ID)
-- 전체 사원들의 사번, 사원명, 부서코드, 부서명
SELECT *
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID;
...
/*
    ...
    단, 반드시 LEFT / RIGHT를 지정해야합니다.(어떤 테이블이 기준이냐)
*/
...
-- 1) LEFT [OUTER] JOIN : 두 테이블 중 왼편에 기술된 데이블을 기준으로 JOIN
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY * 12
FROM EMPLOYEE
...
-- 2) RIGHT [OUTER] JOIN : 두 테이블 중 오른편에 기술된 데이블을 기준으로 JOIN
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY * 12
FROM EMPLOYEE
...
/*
    3. 비등가 조인
    매칭시킬 컬럼에 대한 1조건 작성시 '='를 사용하지 않는 조인문
    ANSI 구문 사용시 JOIN IN만 사용가능
...
-- 전체 사원의 사번, 사원명, 부서코드,            -=> EMPLOYEE E
-- 사수사번, 사수의 사원명, 사수의 부서           -=> EMPLOYEE M
...
/*
    내추럴조인(NATURAL JOIN)
    두 테이블 간에 공통돤 컬럼이름을 자동으로 기분을 정해서 JOIN하는 방식
    ON이나 USING 조건을 지정하지 않아도 두 테이블 간 공통된 커럼을 기반으로 조인
    
    동일한 컬럼이 있을 경우에만 정상적인 조인이 가능하면 명시적이지 않고 조인조건을 정확하게
    기술하기 어렵다.
*/
SELECT *
FROM EMPLOYEE
