/*
    ...
    정수 값을 순차적으로 일정하게 증가 시키면서 생성해준다.
...
/*
    ...
    * 캐시 메모리 : 미리 발생될 값들을 생성해서 저장해두는 공간
                  ...
                  캐시 메모리 공간에 미리 생성된 값들을 가져다 쓸 수 있다. => 속도가 빨라진다.
...
/*
    2. 시퀀스 사용
    시퀀스명.CURRVAL : 현재 시퀀스 값(마지막으로 성공한 시퀀스명.NEXTVAL)
    시퀀스명.NEXTVAL : 시퀀스 값에 일정한 값을 증가시켜 발생한 값
                      현재 시퀀스 값에 INCEMENT BY 값 만큼 증가한 값
*/

SELECT * FROM USER_SEQENCE;

SELECT SEQ_EMPNO.CURRVAL FROM DUAL;
--> NEXTVAL을 한 번도 수행하지 않은 이상 CURRCAL을 할 수 없음
--> 왜? CURRVAL은 마지막으로 성공한 NEXTVAL의 값을 저장해서 보여주는 임시값
...
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 315 에러발생 => 최대값이 310
