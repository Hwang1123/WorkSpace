/*
    ...
    =>테이블은 여러 컬럼을 구성되고, 각 컬럼은 테이블이 표현하는 데이터의 세부적인 속성을 나타냄.
...
--EMPOLOYEE테이블의 사원명, 사원의 연봉(SALARY * 12)를 조회
SELECT EMP_NAME, SALARY *12 
FROM EMPLOYEE;

--EMPLOYEE테이블의 사원명, 급여 ,보너스, 사원의 보너스 포함 연봉((급여 + (급여 * 보너스)) * 12
...
--EMPLOYEE 테이블에서 보든 사원의 월급을 조회한다.
...
--위 주석처럼 작성시 에러가 발생한다. DISTINCT는 명령어에서 한 번만 사용가능
...
-- 1. 급여가 300만원 이상인 사원명, 급여, 입사일, 연봉(별칭 ->연봉) 조회
SELECT EMP_NAME, SALARY, HIRE_DATE, (SALARY * 12) AS "연봉" 
FROM EMPLOYEE
WHERE SALARY >= 3000000;

-- 2. 연봉이 5천만원인 이상인 사원들의 사원명, 급여, 연봉(별칭 ->연봉), 부서코드 조회
...
-- 4. 급여가 350만원 이상 600만원 이하인 모드 사원의 사원명, 사번, 급여 조회
...
/*
    ...
    [표현법]
    비교대상 컬럼 BETWEEN 하한 값 AND 상한 값;
*/

--급여가 350만원 이상 600만원 이하인 모드 사원의 사원명, 사번, 급여 조회
--중간에 AND,OR로 조건을 연결할 수 있다.
SELECT EMP_NAME, EMP_NO, SALARY 
FROM EMPLOYEE
--WHERE SALARY >= 3500000 AND SALARY <= 6000000;
...
/*
    <LIKE>
    비교하자고자 하는 컬럼 값이 내가 제시한 특정 패턴에 만족할 경우 조회
    
    [표현법]
    비교할 대상 컬럼 LIKE '특정패턴'; => 일치하는 것만 가져온다
    
    특정패턴을 제시할 때 와일드 카드라는 특정 패턴이 정의 되어있다.
    1. '%' : 포함문자 검색(0글자 이상 전부 조회)
    EX) 비교할 대상 칼럼 LIKE '문자%' : 비교할 대상 컬럼 값 중에서 해당문자로 시작하는 값들을 전부 가져온다.
        비교할 대상 칼럼 LIKE '%문자' : 비교할 대상 컬럼 값 중에서 해당문자로 끝나는 값들을 전부 가져온다.
        비교할 대상 칼럼 LIKE '&문자%' : 비교할 대상 컬럼 값 중에서 해당문자가 포함 된 값들을 전부 가져온다.
    ...
    EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할 대상 컬럼 값 문자 앞에 아무 글자나 딱 한글자가 있는 값을 조회
        ...
        비교할 대상 컬럼 LIKE '_문자_' : 비교할 대상 컬럼 값 문자 앞 뒤에 아무 글자나 딱 한글자씩 있는 값을 조회
        
        비교할 대상 컬럼 LIKE '_문자_' : 내가 원하는 형태로 _를 통해서 문자수를 조절 할 수 있다.
*/
...
--사원들 중에 전화번호 3번째 자리가 1인 사원들의 사번, 사원명, 전화번호 조회
...
FROM EMPLOYEE
-- WHERE EMAIL LIKE '___%'; => 와일드 카드로 인식되기 때문에 정상적으로 출력할 수 없다.
--와일드 카드와 일반 문자를 구분해줘야 한다.
--데이터 값으로 취급하고 싶은 와일드 카드 문자 앞에 나만의 탈출 문자를 제시해서 탈출시켜주면 된다.
--ESCAP OPTION을 등록해서 사용
WHERE EMAIL LIKE'___/_%' ESCAPE '/';

--===========================================
/*
    <IN>
    WHERE절에 비교대상 컬럼 값이 내가 제시한 목롱 중에 일치하는 값이 있는지를 검사하는 문법
...
/*
    ...
    5. BETWEEN A AND B
    6. NOT
    7. AND --AND와 OR은 같이 쓰임이 많기 때문에 실수하지 않도록 조심
    8. OR
*/
--===================================================
...
-- 2. 전화번호 첫 3자리가 010이 아닌 사원들의 사원명, 전화번화 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE PHONE NOT LIKE '010%';

-- 3. 이름에 '하'가 포함 되어있고 급여가 240만원 이상인 사원들의 사원명, 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY >= 2400000 AND EMP_NAME LIKE '%하%';

-- 4. 부서(DEPT)테이블에서 해외영어부인 부서들의 부서코드, 부서명 조회
SELECT DEPT_ID, DEPT_TITLE
FROM DEPARTMENT
WHERE DEPT_TITLE LIKE '해외%';

-- 5. 사수(MANAGER)가 없고 부서배치도 받지 않은 사원들의 사원명, 사번, 부서코드 조회
SELECT EMP_NAME, EMP_ID, DEPT_CODE, MANAGER_ID
FROM EMPLOYEE
WHERE MANAGER_ID IS NULL AND DEPT_CODE IS NULL;

-- 6. 연봉(급여 * 12)이 3천만원 이상이고 보너스를 받지 않은 사원들의 사번, 사원명, 입사일, 부서코드 조회
