...
-- 3047663보다 더 많은 급여를 받는 사원의 사번, 이름, 직급코드, 급여
...
/*
    * 서브쿼리의 구분
    서브쿼리를 수행한 결과값이 몇행 몊열로 나오느냐에 따라서 분류
    -- 단일행 서브쿼리 : 서브쿼리의 조회 결과값이 오로지 한 개일 때
    -- 다중행 서브쿼리 : 서브쿼리의 조회 결과값이 여러행일 때(여러행 한 열)
...
/*
    1. 단일행 서브쿼리
    서브쿼리 결과로 값이 1개일 때 사용한다(한 행 한 열)
    일반적응로 비교 연산자 사용 가능
    = != > < >= <=
*/
...
--4) 부서별 급여합이 가장 큰 부서의 부서코드, 급여합
    -- 1) 부서별 급여합이 가장 큰 부서의 급여합
SELECT MAX(SUM(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE;

    -- 2) 급여합이 A인 부서의 부서코드 급여합
...
-- 5) '전지연'사원가 같은 부서의 사람들의 사번, 사원명, 전화번호, 입사일, 부서명 조회
-- 단, 전지연 사원 제외
SELECT EMP_ID, EMP_NAME, PHONE, HIRE_DATE, DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
WHERE DEPT_CODE = (SELECT DEPT_TITLE
                    FROM EMPLOYEE
                    WHERE EMP_NAME = '전지연')
AND EMP_NAME != '전지연';
...
/*
    ...
    비교대상 > ANY (서브쿼리) : 여러개의 결과값 중에서 한 개라도 클 경우 조회
    비교대상 < ANY (서브쿼리) : 여러개의 결과값 중에서 한개라도 작을 경우 조회
    비교대상 > ALL (서브쿼리) : 여러개의 모든 결과값이 클 경우 조회
    비교대상 > ALL (서브쿼리) : 여러개의 모든 결과값이 작을 경우 조회
*/

-- 1) 유재석 또는 윤은해 사원과 같은 직급인 사원들의 사번, 사원명, 직급코드, 급여 조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
-- 2) 대리직급임에도 과장 직급의 급여들 중에서
--  가장 최소 급여보다도 많이 받는 사원들의 사번, 이름, 직급코드, 급여 조회
-- 2-1 과장직급의 급여
SELECT SALARY
FROM EMPLOYEE
...
-- 2-2 대리직급을 가졌지만 과장 직급의 급여보다 많은 급여를 받는 사원(대리)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
...
-- 1) 하이유 사원가 같은 부서코드, 같은 직급코드에 해당하는 사원들 조회
-- 사원명, 부서코드, 직급코드, 입사일
...
-- 각 부서별 최고 급여를 받는 사원들의 사번, 사원명, 부서코드
...
-- 각 부서별 급여 합계가 전체 급여 총 합의 20%보다 많은 부서의 부서명, 부서별 급여합계 조회
...
-- 단, 보너스 포함 여봉이 3000만원 이상인 사원들만 조회
...
--=> 인라인뷰를 주로 사용하는 경우 => TOP-N :상위 몇 개만 조회
...
SELECT ROUND(AVG(SALARY)) AS "평균급여", DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
GROUP BY DEPT_TITLE
ORDER BY AVG(SALARY) DESC;

SELECT 평균급여, DEPT_TITLE
FROM (SELECT ROUND(AVG(SALARY)) AS "평균급여", DEPT_TITLE
                FROM EMPLOYEE
...
-- 인라인 뷰
SELECT * 
FROM(SELECT DEPT_CODE, SUM(SALARY), AVG(SALARY) AS "평균급여", COUNT(*)
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
        ORDER BY AVG(SALARY) DESC) 
WHERE 평균급여 > 2700000;
