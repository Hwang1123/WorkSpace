...
/*
    <ORDER BY절>
    SELECT문 가장 마지막 줄에 작성, 상행순서 또한 가장 마지막에 실행한다,
    
    [표현법]
    SELECT 조회할 컬럼...
    FROM 조회할 테이블
    WHERE 조건식
    ORDER BY 정렬기분이 될 컬럼명 | 별칭 | 컬럼순번 [ASC | DESC] [NULLS FIRST | NULLS LAST]
    
    -ASC : 오름차순(작은 값으로 시작해서 점점 커지는 것) => 기본값\
    -DESC : 내림차순(큰 값으로 시작해서 값이 점점 줄어드는 것)
    
    -NULL은 기본적으로 가장 큰 값으로 분루해서 정렬한다.
    -NULLS FRIST : 정렬하고자 하는 컬럼 값에 NULL이 있을 경우 해당 데이터 맨 앞에 배치(DESC일 때 기본 값)
    -NULLS LAST : 정렬하고자 하는 컬럼 값에 NULL이 있을 경우 해당 데이터 맨 뒤에 배치(ASC일 때 기본 값)
...
--ORDER BY BONUS DESC; --NULLS FIRST가 기본 값으로 적용
--정렬 기분의 컬럼 값이 동일할 경우 그 다음차순을 위해서 여러개를 제시할 수 있다.
ORDER BY BONUS DESC, SALARY ASC;

--전 사원의 사원명, 연봉 조회(이 때 연봉을 기준으로 내림차순 정렬)
SELECT EMP_NAME, SALARY * 12 AS "연봉" 
FROM EMPLOYEE 
--ORDER BY SALARY *12 DESC; --선호
--ORDER BY 연봉 DESC;  --선호
ORDER BY 2 DESC; --정렬은 항상 마지막
...
/*
    ...
    -단일행 함수 : N개의 값을 읽어들여서 N개의 결과 값을 리턴(매 행마다 함수 실행 결과를 반환)
    -그룹 함수 : N개의 값을 읽어들여서 1개의 결과 값을 리턴(그룹을 지어 그룹별로 함수 실행결과를 반환)
    
    >>SELECT 절에 단일행 함수와 그룹 함수를 함께 사용하지 못한다.
    이유 : 결과 행의 갯수가 서로 다르기 때문에
...
SELECT INSTR('AABAACAABBAA', 'B', -1) FROM DUAL; --뒤에서 찾지만 읽을 때는 원래대로 읽어준다.
...
/*
    *LPAD / RPAD
    문자열을 조회할 때 통일감 있도록 조회하고자 할 때 사용
    [표현법]
    LPAD / RPAD(STRING, 최종적으로 변환할 문자열의 길이, [덧붙이고자 하는 문자])
*/
...
/*
    LTRIM / RTRIM
    문자열에서 특정 문자를 제거한 나머지를 변환 
    LTRIM / RTRIM(컬럼 | '문자열', [제거하고자 하는 문자들})
    
    문자열의 왼쪽 혹은 오른쪽에서 제거하고자 하는 무자들을 찾아서 제거한 나머지 문자열 반환
*/

SELECT LTRIM('     K  H  ') FROM DUAL; --앞에서부터 다른 문자가 나올 때 까지 공백제거
SELECT RTRIM('123123KH123', '123') FROM DUAL;
SELECT LTRIM('ACABACCAKH', 'ABC') FROM DUAL;

/*
    * TRIM
    문자열의 앞/뒤/양쪽에 있는 저장한 문자들을 제거한 나머지 문자열 반환
    TRIM([LEADING | TRALLING | BOTH] 제거하고자 하는 문자열 FROM 문자열)
*/

SELECT TRIM('     K   H     ') FROM DUAL;
SELECT TRIM(BOTH 'Z' FROM 'ZZZZZZKHZZZZZZZ') FROM DUAL; --양쪽
SELECT TRIM(LEADING 'Z' FROM 'ZZZZZZKHZZZZZZZ') FROM DUAL; --LTRIM유사
SELECT TRIM(TRAILING 'Z' FROM 'ZZZZZZKHZZZZZZZ') FROM DUAL; --RTRIM유사
---------------------------------------------------------------------
...
/*
    * LOWER / UPPER / INITCAP
    ...
    INITCAP : 띄어쓰기 기분 첫 글자마다 대문자로 변경한 문자열 반환
*/

SELECT LOWER('Welcome Ro My KH') FROM DUAL;
SELECT UPPER('Welcome Ro My KH') FROM DUAL;
SELECT INITCAP('Welcome Ro My KH') FROM DUAL;
---------------------------------------------------------------------

/*
    * CONCAT
    문자열 두 개를 전달 받아서 하나로 합친 후 변환
...
SELECT ROUND(123.656) FROM DUAL; --기본자수는 소수점 첫 번째 자리에서 반올림
SELECT ROUND(123.656, 1) FROM DUAL; --양수로 증가할 수록 소수점 뒤로 한 칸씩 이동
SELECT ROUND(123.656, -1) FROM DUAL; -- 음수로 감소할 수록 소수점 앞자리 이동
----------------------------------------------
/*
    * CEIL
    올림처리를 위한 함수
...
-- * ADD MONTHS : 특정 날짜에 NUMBER개월 수를 더해서 반환
SELECT ADD_MONTHS(SYSDATE, 7) FROM DUAL;

-- 사원 테이블에서 사원명, 입사일, 수습기간 종료일을 조회
SELECT EMP_NAME, HIRE_DATE, ADD_MONTHS(HIRE_DATE, 3)
FROM EMPLOYEE;

-- *NEXT_DAY(DATE, 요일(문자 | 1숫자)) : 특정날짜 이후 가장 가까운 요일의 날짜를 반환
...
SELECT TO_CHAR(12, '00000') FROM DUAL; -- 0의 자리수만큼 공간 확보, 빈간을 0으로 채움
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
    TO_CHAR(SYSDATE, 'RRRR'), -- RR룰이 따로 존재한다 => 50년 이상이면 다음세기, 50년 이하면 현재세기
    TO_CHAR(SYSDATE, 'YY'),
...
SELECT 
    ...
    TO_CHAR(TO_DATE('60','RRRR'), 'YYYY'), -- 50년 이상이므로 과거세기로 해석
...
-- 일에 관련된 포멧
SELECT TO_CHAR(SYSDATE, 'DDD'), -- 오늘이 이번 년도에 몇 번째 일수
...
SELECT '1000' + '3000' FROM DUAL; --문자열 => 숫자로 자동 형변환이 진행된다.
...
-- * NVL2(컬럼, 반환 값1, 반환 값2)
-- 반환 값1 : 해당 컬럼이 존재하면 보여줄 값
--반환 값2 : 해당 컬럼이 존재하지 않으면 보여줄 값
...
-- 5. COUNT(* | 컬럼 | DISTINCT 컬럼) : 해당 조건에 맞는 행의 갯수를 세서 반환
    ...
    -- COUNT(DISTINCT 컬럼) : 해당 컬럼값의 중복을 제거한 후 행의 갯수를 세서 반환
...
-- 현재 사원들이 총 몇 개의 부서에 분포되어 있는지를 구하라
