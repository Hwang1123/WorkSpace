...
SELECT EMP_NAME, SALARY, HIRE_DATE, PHONE, SAL_LEVEL
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE (((SALARY * NVL(BONUS,1)) * 12) - ((SALARY * 12) * 0.03)) >= 5000000;
...
-- SELECT EMP_NAME, RPAD(SUBSTR(EMP_NO,1,7), 1, 14, '*') FROM EMPLOYEE;
SELECT EMP_NAME, SUBSTR(EMP_NO,1,7) || '-*******' FROM EMPLOYEE;

--17. EMPLOYEE테이블에서 사원명, 입사일-오늘, 오늘-입사일 조회 
--(단, 각 별칭은 근무일수1, 근무일수2가 되도록 하고 모두 정수(내림), 양수가 되도록 처리)
SELECT EMP_NAME, FOORR(ABD(HIREDATE -SYSDATE)), FOORR(ABS(SYSDATE - HIRE_DATE))
FROM EMPLOYEE;
...
SELECT EMP_NAME, TO_CHAR(SALARY, '0,000,000')
FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE,
        SUBSTR(EMP_NO, 1, 2) || '년' || SUBSTR(EMP_NO, 3, 2) || '월' || SUBSTR(EMP_NO, 4, 2) || '월' AS "생년월일",
