...
-----------------------------------------------------
-- 1-2) 래퍼런스 타입 변수 선언 및 초기화(어떤 테이블의 어떤 컬럼의 데이터 타입을 창조해서 그 타입으로 지정)
DECLARE
...
-- DECLARE
-- 래퍼런스 변수(EID, ENAME, DTILES, NCODE)
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('당신의 점수는 :' || SCORE || '점이며, 학점은' || GRADE || '입니다.');
...
/*
    ...
    * 시스템 예외(오라클에서 미리 정해둔 예외)
    ...
    - TOO_MANY_ROWS : SELECT한 결화가 여러행일 때
