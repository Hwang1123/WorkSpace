...
public class ListRun {
	/*
	 	...
	 	- 자료구조 : 방대한 데이터를 보다 효율적으로 관리(추가, 삭제, 조회, 정렬, 수정)할 수있도록 해주는 구조이다.
	 	- 프레임워크 : 이미 만들어져있는 틀(코드더미)
	 */
	public static void main(String[] args) {
		//ArrayList
		List list = new ArrayList(); // 크기를 지정해도 되고 안해도 됨
		
		System.out.println(list); //빈 배열 상태
		
		//1.add(E e) : 리스트 공간 끝에 전달된 데이터를 추가시켜주는 메서드
		
		list.add(new Music("에피소드", "이무진"));
		list.add(new Music("천상연", "이창섭"));
		list.add(new Music("비의랩소디", "임재현"));
		list.add("끝");
		
		//지정된 크기보다 더 많이 넣어도 에러가 발생하지 않는다. -> 장점1. 크기제약 없음
		...
		//4. set(int index, E e) : 해당 인덱스의 값을 전달받은 e객체로 덮어씌움
		...
		List sub = list.subList(1, 3); //1번 인덱스부터 3번 인덱스 전까지
		...
		//9. isEmpty() : boolean -> 컬렉션이 비어어있는지 확인하는 메서드
