...
public class RunException {
	...
	public void method01() {
		...
		finally { //무조건 한 번은 실행시켜주는 구문
			sc.close();
		}
		System.out.println("프로그램 종료");
	}
	//2. throw : 여기서 예외를 처리하지 않고 현재 이 메서드를 호출한 곳으로 예외처리를 떠넘기겠다.
	public void method02() {
		/*
		 	CheckedException : 반드시 예외처리를 해야되는 예외들
		 	-> 조건문을 미리 제시할 수 없음(왜? 예측이 불가한 곳에서 문제가 발생할 수 있기 때문에)
		 	-> 외부 매개ㅐ체와 입출력이 일어날 때 발생할 수 있음
		 */
		//Scanner와 같이 키보드로 입력받은 값ㅇ르 코드로 가져와주는 객체(단, 문자열로만 읽어들인다.)
	...
	public void methodC() throws IOException {
		throw new IOException("C만의 이유로 에러발생");
	}
	public void methodD() throws CustomException {
		throw new CustomException("D만의 이유로 에러발생"); //예외 직접발생
	}
}
