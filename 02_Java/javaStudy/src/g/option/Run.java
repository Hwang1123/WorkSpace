 ...
 */
 ...
 /*
  	...
  	static변수와 static메서드는 정적메모리를 사용하여 프로그램 실행시점에 따로 객체 생성 없이
  	메모리가 생성되며, 이를 모든 객체가 공유해서 사용한다.
  	
  	클래스(static)변수 : 한 클래스에서 공통적인 값을 유지해야할 때 사용
  	클래스(static)메서드 : 인스턴스 변수를 사용할 수 없으므로 인스턴스와 관계없는 메서드를 
  					   클래스 메서드로 사용한다.
  */
public class Run {
	public static void main(String[] args) {
		/*
		Book b1 = new Book("이게 되네? 챗GPT 미친 활용법 71제","IT","오힘잔",120);
		Book b2 = new Book("Easy! 딥러닝","IT","혁펜하임",140);
		Book b3 = new Book("혼자 공부하는 컴퓨터","IT","박성수",100);
