...
public class Run {
	/*
	 	...
	 	*서버, 클라이언트 연결방식
	 	서버 : 클라이언트(고객)에게 서비스를 제공해주는 프로그램 또는 컴퓨터(요청을 받아서 응답하는 것)
	 	클라이언트 : 서버에 요청을 보내는 컴퓨터(서비스를 제공받는 고객)
	 	
	 	- 서버에 요청하기 위해서는 요청하고자하는 서버의 IP주고, port번호를 알아야함
	 	
	 	도메인 : 특정 서버에 접근하기 위한 정보(ip, port)를 사람이 읽고 기억하기 쉽게 만든 주소
	 			-> 실제 샤용시 DNS에서 ip로 변경해서 위치를 찾는다.
	 			
	 	소켓프로그래밍(TCP/UDP)
	 	- TCP방식 : 데이터의 전송속도가 느려! 데이터가 정확하고 안정적으로 전달이 가능
	 	- UDP방식 : 데이터의 전송속도가 빠름! 신뢰성 없는 데이터 전달에 사용된다.
	 */
