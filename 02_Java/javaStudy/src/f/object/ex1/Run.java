...
public class Run {
	/*
	 	1. 객체 지향 언어 : "객체"를 "지향"하는 언어로 "객체중심"으로 돌아간다.
	 	...
	 	4. 구현하고자 하는 프로그램상의 필요한 객체들을 만들기 위해서(생성하기 위해서)
	 		: 클래스라는 설계도가 먼저 필요하다.
	 	
	 	5. 추상화 과정
	 		1) 내가 구현하고자 하는 플그램에서 필요한 객체가 뭘까? 생각해봐야한다.
	 		...
	 		ex) 학생관리 프로그램
	 		1) 학생관련 객체(김민수 훈련생...최지원 훈련생...김민지 훈련생..등등)
	 		2) 공통적인 속성 및 기능(이름, 나이, 주소, 전화번호 등등...)
	 		3) 학생 "인적사항"을 관리하는 프로그램 : 이름, 나이, 주소, 전화번호....
	 		   학생 "성적"을 관리하는 프로그램 : 이름, 반, 과목별 점수, 성장률...
	 		   학생 "건강"을 관리하는 프로그램 : 이름, 반, 달리기 속도, 점프력, 몸무게....
	 		4) 이름 : String name;
	 		   나이 : int age;
	 		   키 : double height;
	 	...
	 	6. 추상화한 객체를 프로그램에 적용시키기
	 		...
	 		2) "배열"로 프로그램을 만든다.
	 			...
	 			이름들 보관하는 배열 String[] name = {"최지원", "김지원"};
	 			나이를 보관하는 배열 int[] age = {55, 60...}	=> name과 age배열 간의 순서를 보장하지 않는다.
	 		
	 		3) 그래서 나온 개념 -> 구조체
	 		   구조체 : 여러개의 자료형의 여러개의 값들을 보관할 수있다. -> 나만의 타입을 만들 수 있음
	 		   Struct human{
	 		   		String name;
	 		   		int age;
	 		   }
	 		   ...
	 		   -> 자바에서는 구조체 개념에 좀 더 확장해서 기능(메서드)을 추가한다 => 클래스  
