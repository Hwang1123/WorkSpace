...
public class B_Array {
	public static void main(String[] args) {
		...
		System.out.println(iArr.hashCode()); // 주소값의 10진수 해시코드
		
		int[] arr = null; //아무것도 참조하고 있지 않음
		System.out.println(arr);
		/*
			arr.hashCode()
			null을 가지고 있는 참조변수에 접근해서 다른 정보를 구하고하자면(함수를 실행하면)
			항상 오류가 발생한다.
		...
		*/
		 ...
		 */
		arr = null;
		//배열을 강제로 삭제시키고자 한다면 null을 대입해주면 된다.
		
		int[] arr2 = {1,1,1,1,1};
		int[] arr3 = {1,1,1,1,1};
		System.out.println(arr2 == arr3);
		// false인 이유 -> 각 참조변수는 == 을 통해서 비교시 주소 값을 비교하기 때문(arr2와 arr3의 주소값이 다르기 때문)
